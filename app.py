...
AI_DISCLAIMER = (
    "Disclaimer: This response is generated by an AI bot from official JAC website data when available. "
    "It may be inaccurate due to website changes, network issues, or parsing errors. "
    "Please verify the final result/marksheet on the official JAC portal."
)
...
def send_telegram_message(chat_id: int, text: str) -> None:
    send_telegram_message_with_buttons(chat_id, text, include_owner_buttons=False)


def send_telegram_message_with_buttons(chat_id: int, text: str, include_owner_buttons: bool) -> None:
...
def start_result_lookup(chat_id: int) -> str:
    if not class_12_live():
        ...
        return (
            "The official JAC Class 12 result is not live yet, so I cannot start direct result checking right now. "
            "You can ask: result live or not"
        )
    ...
    if step == "await_format":
        ...
        if not result_entry_live():
            return (
                ...
                "so I cannot fetch the result directly right now. As a fallback, I can still share text updates here once live.\n\n"
                f"{AI_DISCLAIMER}"
            )
        ...
        if format_choice not in supported_modes:
            return (
                ...
                "available from the verified JAC flow yet. I will fall back to text format if result data can be read.\n\n"
                f"{AI_DISCLAIMER}"
            )
        ...
        return (
            ...
            "summary delivery or direct PDF sending depending on what JAC returns. If neither works, I will fall back to text format.\n\n"
            f"{AI_DISCLAIMER}"
        )
...
def telegram_webhook():
    ...
    if text and chat_id:
        ...
        if lookup_reply is not None:
            ...
            reply = (
                f"This bot was created by {OWNER_NAME}. "
                "You can use the button below to visit the Instagram profile."
            )
            include_owner_buttons = True
        else:
            reply = build_reply(text)
            include_owner_buttons = False
        send_telegram_message_with_buttons(chat_id, reply, include_owner_buttons)
